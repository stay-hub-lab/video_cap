...
case $interrupt_selection in
	"0")
		...
		ret=`insmod "${SCRIPT_DIR}/xdma/xdma.ko" interrupt_mode=0`
		;;
	"1")
		echo "insmod xdma.ko interrupt_mode=1 ..."
		ret=`insmod "${SCRIPT_DIR}/xdma/xdma.ko" interrupt_mode=1`
		;;
	"2")
		echo "insmod xdma.ko interrupt_mode=2 ..."
		ret=`insmod "${SCRIPT_DIR}/xdma/xdma.ko" interrupt_mode=2`
		;;
	"3")
		echo "insmod xdma.ko interrupt_mode=3 ..."
		ret=`insmod "${SCRIPT_DIR}/xdma/xdma.ko" interrupt_mode=3`
		;;
	"4")
		echo "insmod xdma.ko poll_mode=1 ..."
		ret=`insmod "${SCRIPT_DIR}/xdma/xdma.ko" poll_mode=1`
		;;
	*)
