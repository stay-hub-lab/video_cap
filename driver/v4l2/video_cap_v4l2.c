...
/*
 * If xdma_vendor/xdma_device are 0, bind the first PCI function that is
 * currently driven by the "xdma" driver (use xdma_index to choose among many).
 */
static unsigned short xdma_vendor;
module_param(xdma_vendor, ushort, 0644);
MODULE_PARM_DESC(xdma_vendor, "XDMA PCI vendor ID (0 means any, default 0)");

static unsigned short xdma_device;
module_param(xdma_device, ushort, 0644);
MODULE_PARM_DESC(xdma_device, "XDMA PCI device ID (0 means any, default 0)");
...
static struct pci_dev *video_cap_find_xdma_pdev(u16 vendor, u16 device, unsigned int instance)
{
	...
	for_each_pci_dev(pdev) {
		/*
		 * Only consider PCI functions that are actually bound to the
		 * xdma driver. This avoids accidentally binding to other Xilinx
		 * functions with the same vendor ID.
		 */
...
static void video_cap_dump_xdma_pci_devs(void)
{
	struct pci_dev *pdev;

	pr_info(DRV_NAME ": scanning PCI for xdma-bound functions...\n");
	for_each_pci_dev(pdev) {
		if (!pdev->driver || strcmp(pdev->driver->name, "xdma"))
			continue;
		pr_info(DRV_NAME ":  %s %04x:%04x (driver=%s)\n",
			dev_name(&pdev->dev), pdev->vendor, pdev->device,
			pdev->driver ? pdev->driver->name : "(none)");
	}
}
...
static int video_cap_bind_xdma(struct video_cap_v4l2_dev *dev)
{
	...
	if (!dev->pdev) {
		pr_err(DRV_NAME ": no matching xdma PCI device (vendor=0x%04x device=0x%04x index=%u)\n",
		       xdma_vendor, xdma_device, xdma_index);
		video_cap_dump_xdma_pci_devs();
		return -ENODEV;
	}

	xpdev = dev_get_drvdata(&dev->pdev->dev);
	if (!xpdev || xpdev->magic != MAGIC_DEVICE) {
		pr_err(DRV_NAME ": %s has no valid xdma drvdata\n", dev_name(&dev->pdev->dev));
	...
	if (!xdev || xdev->magic != MAGIC_DEVICE) {
		pr_err(DRV_NAME ": %s has no valid xdma_dev\n", dev_name(&dev->pdev->dev));
		pci_dev_put(dev->pdev);
		dev->pdev = NULL;
		return -ENODEV;
	}
	...
	if (dev->c2h_channel >= xdev->c2h_channel_max) {
		pr_err(DRV_NAME ": invalid c2h_channel=%u (max=%d)\n",
		       dev->c2h_channel, xdev->c2h_channel_max);
		pci_dev_put(dev->pdev);
		dev->pdev = NULL;
		return -EINVAL;
	}
	if (dev->irq_index >= (unsigned int)xdev->user_max) {
		pr_err(DRV_NAME ": invalid irq_index=%u (user_max=%d)\n",
		       dev->irq_index, xdev->user_max);
		pci_dev_put(dev->pdev);
		dev->pdev = NULL;
		return -EINVAL;
	}
	...
	if (xdev->user_bar_idx < 0 || xdev->user_bar_idx >= XDMA_BAR_NUM ||
		...
		pr_err(DRV_NAME ": user BAR not mapped (user_bar_idx=%d)\n",
		       xdev->user_bar_idx);
		pci_dev_put(dev->pdev);
		dev->pdev = NULL;
		return -ENODEV;
...
static int video_cap_wait_vsync(struct video_cap_v4l2_dev *dev)
{
	...
	rv = wait_event_interruptible_timeout(user_irq->events_wq,
					      READ_ONCE(dev->stopping) ||
						      READ_ONCE(user_irq->events_irq) != 0,
...
static int video_cap_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	/* Buffers may have been queued before STREAMON; ensure the thread runs. */
	wake_up(&dev->wq);
